...
def addSubtitleInfo(metadata, state):
    metadata["subtitles"] = {"fonts": [], "streams": []}
...
def detect_stream(stream):
    if not "codec_name" in stream or stream["codec_name"] == "attachment":
        if "tags" in stream and "mimetype" in stream["tags"]:
            if (
                stream["tags"]["mimetype"] == "application/x-truetype-font"
                or stream["tags"]["mimetype"] == "application/vnd.ms-opentype"
            ):
                return {
                    "type": "font",
        ...
        if stream["codec_name"] == "gif":
            return {
                ...
                "flags": {"autoplay": True, "loop": True, "mute": True},
            }
    if stream["codec_type"] == "video":
        return {
            ...
            "flags": {"autoplay": False, "loop": False, "mute": False},
        }
