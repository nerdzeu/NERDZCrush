...
class APITestCase(TestMixin):
    ...
    def _upload(self, f, ip="127.0.0.1"):
        return self._post(
            "/api/upload/file", {"file": (open("test_data/%s" % f), f)}, ip=ip
        )
    ...
    def _get_url_hash(self, url):
        result = self._post("/api/upload/url", {"url": url,}, ip="127.0.0.1")
...
class AlbumTestCase(APITestCase):
    def test_create_album(self):
        h = [self._get_hash("cat.png"), self._get_hash("cat2.jpg")]
    ...
    def test_album_issue_422(self):
        ...
        self.client.delete("/api/%s" % h[0], environ_base={"REMOTE_ADDR": "127.0.0.1"})
    ...
    def test_create_album_bad_hash(self):
        h = [self._get_hash("cat.png"), "sdfgksgflg"]
    ...
    def test_create_album_containing_album(self):
        ...
        response = self._create_album(h)
        h.append(
            json.loads(response.data)["hash"]
        )  # Add the previous album's hash to the list
    ...
    def test_delete_album(self):
        ...
        response = self.client.delete(
            "/api/%s" % album, environ_base={"REMOTE_ADDR": "127.0.0.1"}
        )
        self.assertEqual(response.status_code, 200)
...
class DeleteTestCase(APITestCase):
    def test_delete(self):
        h = self._get_hash("cat.png")
        response = self.client.delete(
            "/api/%s" % h, environ_base={"REMOTE_ADDR": "127.0.0.1"}
        )
    ...
    def test_delete_bad_ip(self):
        h = self._get_hash("cat.png")
        response = self.client.delete(
            "/api/%s" % h, environ_base={"REMOTE_ADDR": "127.0.0.2"}
        )
...
class FlagsTestCase(APITestCase):
    def test_correct_flags_gif(self):
        ...
        self.assertEqual(
            obj["flags"],
            {u"autoplay": True, u"loop": True, u"mute": True, u"nsfw": False},
        )
    ...
    def test_correct_flags_mp4(self):
        ...
        self.assertEqual(
            obj["flags"],
            {u"autoplay": False, u"loop": False, u"mute": False, u"nsfw": False},
        )

    def test_change_flags(self):
        h = self._get_hash("cat.gif", wait_ready=True)
        response = self._post("/api/%s/flags" % h, {"autoplay": False})
    ...
    def test_change_flags_unauthorised(self):
        h = self._get_hash("cat.gif", wait_ready=True)
        response = self._post("/api/%s/flags" % h, {"autoplay": False}, ip="127.0.0.2")

        self.assertEqual(response.status_code, 401)

    def test_change_extraneous_flags(self):
        h = self._get_hash("cat.gif", wait_ready=True)
        response = self._post("/api/%s/flags" % h, {"autoplay": False, "dummy": True,})

        self.assertEqual(response.status_code, 415)

    def test_change_flags_404(self):
        response = self._post("/api/%s/flags" % "dummy", {"autoplay": False,})
