...
from mediacrush.network import get_ip, is_tor, secure_ip
from mediacrush.objects import Album, FailedFile, Feedback, File, RedisObject
...
deletion_procedures = {File: delete_file, Album: lambda a: a.delete()}
...
class APIView(FlaskView):
    ...
    def status(self, h):
        ...
        ret = {"status": f.status}
        if (
            f.processor is not None
        ):  # When processor is available, ther rest of the information is too, even if the file might not have finished processing yet.
